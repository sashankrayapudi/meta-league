...
async function getLeague(req, res) {
  ...
  const yearlyRosterProjections = {}
  for (rosterId in rosterProjections) {
    yearlyRosterProjections[rosterId] = 0
    for (player of rosterProjections[rosterId]){
      for (week in player.wp) {
        yearlyRosterProjections[rosterId] += player.wp[week].p
      }
    }
  }

  const yearlyProjectionsData = []
  for (rosterId in yearlyRosterProjections) {
    yearlyProjectionsData.push({rosterId: rosterId, points: yearlyRosterProjections[rosterId]})
  }
  
  for (let i = 0; i < yearlyProjectionsData.length; i++) {
    yearlyProjectionsData[i]['teamName'] = usersInLeagueData[i].display_name
  }
